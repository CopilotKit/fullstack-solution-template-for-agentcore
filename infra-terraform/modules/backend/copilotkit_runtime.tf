...
# =============================================================================
# CopilotKit Runtime Lambda + API Gateway
# Bridges CopilotKit frontend protocol to AgentCore AG-UI endpoint.
# =============================================================================
...
resource "null_resource" "copilotkit_runtime_build" {
  triggers = {
    source_hash = sha256(join("", [
      for f in fileset(local.copilotkit_runtime_source_path, "src/**")
...
resource "aws_lambda_function" "copilotkit_runtime" {
  ...
  environment {
    variables = {
      AGENTCORE_AG_UI_URL = "https://bedrock-agentcore.${local.region}.amazonaws.com/runtimes/${urlencode(aws_bedrockagentcore_agent_runtime.main.agent_runtime_arn)}/invocations?qualifier=DEFAULT"
    }
  }
...
resource "aws_apigatewayv2_route" "copilotkit_root_get" {
  api_id    = aws_apigatewayv2_api.copilotkit_runtime.id
  route_key = "GET /copilotkit"
  target    = "integrations/${aws_apigatewayv2_integration.copilotkit_runtime.id}"
}

resource "aws_apigatewayv2_route" "copilotkit_root_post" {
  api_id    = aws_apigatewayv2_api.copilotkit_runtime.id
  route_key = "POST /copilotkit"
  target    = "integrations/${aws_apigatewayv2_integration.copilotkit_runtime.id}"
}

resource "aws_apigatewayv2_route" "copilotkit_proxy_get" {
  api_id    = aws_apigatewayv2_api.copilotkit_runtime.id
  route_key = "GET /copilotkit/{proxy+}"
  target    = "integrations/${aws_apigatewayv2_integration.copilotkit_runtime.id}"
}

resource "aws_apigatewayv2_route" "copilotkit_proxy_post" {
  api_id    = aws_apigatewayv2_api.copilotkit_runtime.id
